-- Hegemony PvP: todo personagem volta ao ponto de partida.
--
-- REGRAS (18/09/2026, com o usuario)
--   - toda conta nasce com 5 personagens: MS, ED, RP e dois EK (um com Magic
--     Sword e escudo, outro com Avenger), nomes aleatorios, em Venore, nivel
--     80 (docker/hegemony/03-contas-com-personagens.sql);
--   - matar da experiencia (experienceByKillingPlayers, no entrypoint), mas o
--     ganho vale so ate deslogar: DESLOGAR devolve nivel, magic level e skills
--     ao molde. Os itens ficam;
--   - MORRER derruba TUDO no corpo e devolve o molde com um kit novo.
--
...
--   3. O "Ok" da tela de morte do nosso cliente faz logout e login (onLogin);
--      o cliente oficial pede Player::spawn, que REAPROVEITA o personagem da
--      memoria e so chama onChangeZone. O reset roda nos dois.
--
...
-- Marca do segundo EK da conta, o da Avenger. Posta pelo SQL que cria os
...
local MOLDES = {
	...
	[8] = { -- Elite Knight: melee 95 nas tres armas
		...
		slots = {
			...
			[CONST_SLOT_RIGHT] = 11688, -- shield of corruption (nivel 80, so EK)
			[CONST_SLOT_FEET] = 3079, -- boots of haste
		},
		-- O EK da Avenger: ela e de duas maos, entao a mao do escudo fica vazia.
		avenger = {
...
local function slotsDoKit(player, m)
	local slots = {}
	for slot, id in pairs(m.slots) do
		slots[slot] = id
	end
	if m.avenger and player:getStorageValue(Hegemony.STORAGE_EK_AVENGER) == 1 then
		for slot, id in pairs(m.avenger) do
			slots[slot] = id or nil
		end
	end
	return slots
end
...
-- Nivel 80, vida/mana/cap do 80, ML e skills do molde, vida e mana cheias.
-- setLevel so troca nivel e experiencia; o resto e posto aqui. Nao mexe em
-- itens: e o que roda ao deslogar e ao entrar.
function Hegemony.voltarAoMolde(player)
...
function entrada.onLogin(player)
	...
	elseif not kv(player):get(KV_NASCEU) and semNadaEquipado(player) then
		-- Personagem novo sai do Sample sem itens.
		Hegemony.renascer(player)
	else
		-- Personagem antigo fica com os itens dele; o nivel volta ao molde
		-- em todo login (rede de seguranca do reset no logout, que nao roda
...
local saida = CreatureEvent("HegemonyMoldeLogout")
function saida.onLogout(player)
	if not Hegemony.livre(player) then
		Hegemony.voltarAoMolde(player)
	end
	return true
end
