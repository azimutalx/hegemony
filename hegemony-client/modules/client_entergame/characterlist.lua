...
local function tryLogin(charInfo, tries)
    ...
    g_logger.info(string.format("Attempting loginWorld: char=%s, host=%s, port=%d, clientVer=%d, sessionKeyLength=%d", charName, host, port, clientVer, #sessionKey))

    local protocolVer = g_game.getClientProtocolVersion(clientVer)
    g_game.chooseRsa(host)
    g_game.setClientVersion(clientVer)
    g_game.setProtocolVersion(protocolVer)

    g_logger.info("Before loginWorld: isLogging=" .. tostring(g_game.isLogging()) .. ", isOnline=" .. tostring(g_game.isOnline()))
    g_game.cancelLogin()
...
function onGameLoginError(message)
    g_logger.error("onGameLoginError: " .. tostring(message))
    CharacterList.destroyLoadBox()
...
function onGameSessionEnd(reason)
    g_logger.info("onGameSessionEnd: " .. tostring(reason))
    CharacterList.destroyLoadBox()
    CharacterList.showAgain()
end

function onGameConnectionError(message, code)
    g_logger.error("onGameConnectionError: " .. tostring(message) .. " code: " .. tostring(code))
    CharacterList.destroyLoadBox()
...
function CharacterList.doLogin()
    removeAutoReconnectEvent()
    local selected = characterList:getFocusedChild()
    if not selected or (not selected.characterName and not selected.characterInfo) then
    ...
    if selected then
        if not selected.characterName and selected:getParent() then
            selected = selected:getParent()
        end
        local charName = selected.characterName
        if not charName and selected.characterInfo then
            charName = selected.characterInfo.name
        end
